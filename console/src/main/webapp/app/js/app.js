...
App.run(["$rootScope", "$state", "$stateParams",  '$window', '$templateCache', function ($rootScope, $state, $stateParams, $window, $templateCache) {
  ...
  /*
	 ...
	 * fromState, fromParams) { if (typeof(toState) !== 'undefined'){
...
function ($stateProvider, $locationProvider, $urlRouterProvider, helper) {
  ...
  $stateProvider
    ...
    .state('app.dashboard', {
        ...
        resolve: angular.extend(helper.resolveFor('flot-chart','flot-chart-plugins','ui.grid','ngDialog','xeditable'),{
    ...
    .state('crm.dashboard_v0', {
        url: '/dashboard_v0',
        title: 'Dashboard v0',
        templateUrl: helper.basepath('dashboard_v0.html'),
        resolve: angular.extend(helper.resolveFor('flot-chart','flot-chart-plugins','ui.grid','ngDialog','xeditable'),{
        	 tpl: function() { return { path: helper.basepath('ngdialog-template.html') }; }
        })
    })
  ...
  .constant('APP_REQUIRES', {
    ...
    scripts: {
      ...
      'jquery-ui':          ['vendor/jquery-ui/ui/core.js',
                             ...
                             // loads only jquery required modules and touch
								// support
    ...
    if($scope.loginForm.$valid) {
      ...
      $http
        ...
        .then(function(response) {
          // assumes if ok, response is an object with some data, if not, a
			// string with error
      ...
      // set as dirty if the user click directly to login so we show the
		// validation messages
...
App.controller('CalendarController', ['$scope', function($scope) {
  ...
  /**
	 ...
	 * 
	 * @param jQuery
	 *            Object elements Set of element as jQuery objects
	 */
  ...
  /**
	 ...
	 * 
	 * @param jQuery
	 *            [calElement] The calendar dom element wrapped into jQuery
	 * @param EventObject
	 *            [events] An object with the event list to load when the
	 *            calendar displays
	 */
  function initCalendar(calElement, events) {
      ...
      calElement.fullCalendar({
          ...
          drop: function(date, allDay) { // this function is called when
              ...
              // clone the object to avoid multiple events with reference to
				// the same object
  ...
  /**
	 ...
	 * Wrap into this function a request to a source to get via ajax the stored
	 * events
	 * 
...
// -----------------------------------
  ...
  $scope.pieOptions = {
    ...
    segmentStrokeColor : '#fff',
    segmentStrokeWidth : 2,
    percentageInnerCutout : 0, // Setting this to zero convert a doughnut into
								// a Pie
    animationSteps : 100,
    animationEasing : 'easeOutBounce',
    animateRotate : true,
...
App.controller('FormDemoCtrl', ["$scope", "$resource", function($scope, $resource) {
  ...
  // the following allow to request array $resource instead of object
	// (default)
...
App.controller('PanelsCtrl', ['$scope', '$timeout', function ($scope, $timeout) {
  ...
  $scope.$on('panel-remove', function(event, id, deferred){
    ...
    // Here is obligatory to call the resolve() if we pretend to remove the
	// panel finally
...
App.controller('CrmController',CrmController);
CrmController.$inject = ['$rootScope', '$scope', '$state', '$translate', '$window', '$localStorage', 
    '$timeout','$http','toggleStateService', 'colors', 'browser', 'cfpLoadingBar',
    'uiGridConstants','ngDialog','editableOptions', 'editableThemes'];
function CrmController($rootScope, $scope, $state, $translate, $window, $localStorage,  
        $timeout,$http,toggle, colors, browser, cfpLoadingBar,
        uiGridConstants,ngDialog,editableOptions,editableThemes){
	 ...
	 var httpConfig = {
		...
		rootUrl : "http://localhost:8088/console/crm"
	 };
	 
	 
	 var htmlConfi = {
	    "table":"app/views/crm/table_html.html",
	    "console":"app/views/crm/console_html.html"
	 };
	 ...
	 //跳转到选择场景页面
	 $scope.goSceneSelectedPage = function(){
		 restScene();
		 ngDialog.open({
	          template: "app/views/ngdialog-scene.html",
	          width: 600,
	          className: 'ngdialog-theme-default',
	          scope: $scope
	     });
	 ...
	 //根据条件筛选场景返回列表 
	 $scope.searchValObj = {"searchVal":""};
	 $scope.getSceneList = function(){
    	 var param = {"searchVal":$scope.searchValObj.searchVal};
    	 $http.post(httpConfig.rootUrl + '/getSceneList',param,httpConfig.config).success(function(response){
    		 console.log(response);
    		 $scope.scene.sceneList = response;
         });
	 }
	 
	 //选择当前场景
	 $scope.getChoseScene = function(item){
		 $scope.scene.curScene = item;
	 }
	 
	 //返回当前选择场景
	 $scope.returnCurScene = function(){ 
		 if($scope.scene.curScene == null || angular.equals({}, $scope.scene.curScene)){
			 alert("请选择一个场景")
		 }
		 
		 $scope.scene.formScene = $scope.scene.curScene;
		 
		 console.log($scope.scene.formScene);
		 
		 var length = parseInt($scope.scene.formScene.p_num);
		 for(var i=0;i<length;i++){
			 $scope.scene.pNumList.push({"key":i,"value":""});
		 }
		 
		 $scope.scene.disPlayName = $scope.scene.formScene.name + " " + $scope.scene.formScene.desc;
		 
	 };
	 
	 //取消选择当然场景
	 $scope.chanelCurScene = function(){
		 $scope.scene.curScene = {};
	 };
	 ...
	 $scope.doExecurte = function(){
		 var param = {"formScene":$scope.scene.formScene,"pNumList":$scope.scene.pNumList};
		 $http.post(httpConfig.rootUrl+'/doExecurte'+'?scene_id='+$scope.scene.formScene.id ,param,httpConfig.config).success(function(response){
    		 console.log(response); 
    		 
    		 restSceneTable();
    		 if(response.resultCode == "1" ){
    			 alert(response.msg);
    		 }else if(response.resultCode == "proc"){
    			 returnDateDealByOptype(response.msg);
    		 }else{
    			 returnDateDealByOptype(response);
    		 }
         });
	 }
	 
	 //根据操作类型 执行填充数据 或者 直接现实
	 function returnDateDealByOptype(data){
		 var opType = $scope.scene.formScene.op_type;
		 if(data == undefined){
			 alert("返回数据错误");
			 return;
		 }
		 ...
		 if(opType == "select"){
			 ...
			 if(length > 0){
				 var first = data[0];
				 for(var atr in first){
					 $scope.sceneTable.header.push(atr);
				 }
				 
				 $scope.sceneTable.data = data; 
			 }
		 }else{
			 $scope.scene.disConsole = data;
		 }
	 }
	 
	 //根据执行optype 区分页面
	 $scope.getHtmlByType = function(){
		 var html = htmlConfi.console;
		 var opType = $scope.scene.formScene.op_type;
		 if(opType == "select"){
			 html = htmlConfi.table;
		 }
		 return html;
	 };
	 ...
	 //预执行测试
	 $scope.aa = function(){
		 console.log($scope.scene.pNumList);
	 }
...
AppController.$inject=['$rootScope', '$scope', '$state', '$translate', '$window', '$localStorage', 
	                   '$timeout','$http','toggleStateService', 'colors', 'browser', 'cfpLoadingBar',
	                   'uiGridConstants','ngDialog','editableOptions', 'editableThemes'];          
function AppController($rootScope, $scope, $state, $translate, $window, $localStorage,  
		               $timeout,$http,toggle, colors, browser, cfpLoadingBar,
		               uiGridConstants,ngDialog,editableOptions,editableThemes) {
    "use strict";
   ...
   // $scope.tpl = tpl;
    ...
    var rootUrl = "http://localhost:8088/console/service";
    
    //接口基础配置
    $scope.baseServiceConfig = {
    		"secondCode":"BUS002002",
    		"thirdCode":"",
    		"contractVersion":"SVC1000120100406",
    		"dfFromCompCode":"6090010023",
    		"dfToCompCode":"6090010001",
    		"dfUrl":"",
    		"serviceReq":"request",
    		"serviceTmo":10,
    		"urls":["http://172.19.17.152:8888/CrmWebService/order?wsdl",
    			    "http://172.19.17.152:8888/CrmWebService/customer?wsdl",
    			    "http://172.19.17.152:8888/CrmWebService/sr?wsdl"
    		]
    };
  
    $scope.baseServiceConfig.dfUrl = $scope.baseServiceConfig.urls[0];
    
    /*$scope.testaa = function(){
    	alert($scope.baseServiceConfig.dfUrl);
    }*/
    ...
    $scope.gridOptionsComplex = {
        ...
        columnDefs: [
            { field: 'SERVICEID', name:"serviceId", displayName:"接口编号", width: '5%' },
            { field: 'BIZFUNCTIONID', name:"bizFunctionId",displayName:"功能编号", width: '5%' },
            { field: 'SERVICEENNAME', name:"serviceEnName",displayName:"英文名称", width: '23%' },
            { field: 'SERVICECNNAME', name:"serviceCnName",displayName:"中文名称", width: '33%' }
        ],
        data: data,
 ...
 // complex grid ---------------------
    ...
    // -- 查询数据
    $scope.searchForm = {"searchVal":""};
    
    $scope.queryAllService = function(){
    	 var url = rootUrl + '/findAllService';
    	 var param = {"searchVal":$scope.searchForm.searchVal};
    	$http.post(url,param,config).success(function(response){
        	$scope.gridOptionsComplex.data = response;
        });
    };     
    
    // 修改接口页
    $scope.goEditServicePage = function () {
        $scope.value = true;

        if($scope.selectedRow == null || angular.equals({}, $scope.selectedRow)){
        	alert("请先选择一行");
        	return;
        }
        
        ngDialog.open({
          template: "app/views/ngdialog-service-edit.html",
          width: 600,
          className: 'ngdialog-theme-default',
          scope: $scope
        });
      ...
      // 修改接口保存
      $scope.doServiceEditSave = function(){
    	// console.log($scope.selectedRow);
      
        return ;
        // var url = rootUrl + '/doServiceEditSave';
        var url = '';
     	var param = $scope.selectedRow;
      ...
      // 删除接口
      $scope.doSerivceDeleteSave = function(){
         // console.log($scope.selectedRow);
        ...
        if($scope.selectedRow == null || angular.equals({}, $scope.selectedRow)){
         ...
         if(!confirm("您确定要删除此:"+$scope.selectedRow.SERVICECNNAME+"接口吗?")){
           return false;
         }
          
         return;
         // var url = rootUrl + '/doSerivceDeleteSave';
         var url = '';
         var param = $scope.selectedRow;
      ...
      // 修改属性页
      $scope.selectedRowAttr = {"seletedRowAttrValues":[]};
      $scope.goEditServiceAttrPage = function(){
    	  // console.log($scope.selectedRow);
          ...
          ngDialog.open({
              template: "app/views/ngdialog-service-attr-edit.html",
              width: 600,
              className: 'ngdialog-theme-default',
              scope: $scope,
              controller : ['$scope','$http',function($scope,$http){
            	var url = rootUrl + '/goEditServiceAttrPageAndGetAttrValues';
               	var param = $scope.selectedRow;
               	$http.post(url,param,config).success(function(response){	
               		$scope.selectedRowAttr.seletedRowAttrValues = response;
               	});
      ...
      // 修改属性保存
      $scope.doServiceAttrValueEditSave = function(){
    	  // console.log($scope.selectedRowAttr.seletedRowAttrValues);
    	  var url = rootUrl + '/doServiceAttrValueEditSave';
          var param = $scope.selectedRowAttr;
      ...
      // 接入权限修改
      $scope.interfaceForm = {"compments": [],"interfaces": []};
      $scope.currentInterface = {"comp":"","inter":"","compFilter":"","interFilter":""};
      $scope.goEditServiceInterfacePage = function(){
    	  if($scope.selectedRow == null || angular.equals({}, $scope.selectedRow)){
            	alert("请先选择一行");
            	return;
          }
          ...
          ngDialog.open({
            template: "app/views/ngdialog-service-interface-edit.html",
            width: 600,
            height: 500,
            className: 'ngdialog-theme-default',
            scope: $scope,
            controller : ['$scope','$http',function($scope,$http){
            	var url = rootUrl + '/goEditServiceInterfacePageAndGetInterfaceAndCompmentsValues';
             	var param = $scope.selectedRow;
             	$http.post(url,param,config).success(function(response){	
             		//console.log(response);
             		$scope.interfaceForm.compments = response.compments;
             		$scope.interfaceForm.interfaces = response.interfaces;
             	});
            }]
          });
      }
      ...
      //双击添加新权限
      $scope.choseComp = function(item){
    	  console.log(item);
    	  $scope.currentInterface.comp = item;
    	  
    	  //js 验证是否存在
    	  var isExists = false;
    	  angular.forEach($scope.interfaceForm.interfaces,function(value,key){
    		  if(isExists == false){
    			  if(value.CODE.indexOf(item.COMPONENT_ID) > -1){
        			  alert("已经添加改平台，禁止重复添加");
        			  isExists = true;
        		  }
    		  }
    	  });
    	  ...
    	  //后台添加平台权限 
    	  var url = rootUrl + '/insertServiceBizInterface';
       	  var param = {"serviceId": $scope.selectedRow.SERVICEID,
       			       "secondCode": $scope.baseServiceConfig.secondCode,
       			       "thirdCode": $scope.baseServiceConfig.thirdCode,
       			       "contractVersion": $scope.baseServiceConfig.contractVersion,
       			       "interfaceF": $scope.currentInterface.COMPONENT_ID
       	              };
       	  
       	  /*if(param.thirdCode == "" || param.thirdCode == undefined){
       		  alert("3级能力编码为空，请到基础设置页面获取。");
       		  return ;
       	  }*/
       	  
       	  if(param.interfaceF == "" || param.interfaceF == undefined){
       		  alert("没有选择中要添加的平台");
       		  return ;
       	  }       	  
       	  ...
       	  $http.post(url,param,config).success(function(response){	
       		  ...
       		  if(response != undefined){
       			 $scope.interfaceForm.interfaces.add(response);       		
       		  }
       	  });
      ...
      //双击移除权限
      $scope.removeInterface = function(item){
    	  console.log(item);
    	  $scope.currentInterface.inter = item;
    	  
    	  if(!confirm("确定要移除当前权限？")){
    		  return ;
    	  }
    	  
    	  if($scope.currentInterface.inter == undefined){
    		  alert("请双击选择移除");
    		  return;
    	  }
    	  
    	  var url = rootUrl + '/removeInterface';
    	  var param = $scope.currentInterface.inter;
    	  
    	  $http.post(url,param,config).success(function(response){	
       		  console.log(response);
       		  if(response.resultSize == "1"){
       			  angular.forEach($scope.interfaceForm.interfaces,function(obj){
       				  if(obj.BIZ_INTERFACE_ID == $scope.currentInterface.inter.BIZ_INTERFACE_ID){
       					  var index = $scope.interfaceForm.interfaces.indexOf(obj);
       					  if(index != undefined && index > -1){
       						$scope.interfaceForm.interfaces.splice(index,1);
       					  }
       				  }
       			  });
       		  }
      ...
      //获得三级能力编码
      $scope.findServiceThirdCode = function(){
    	  var url = rootUrl + '/findServiceThirdCode';
       	  var param = {"a":"a"};
       	  $http.post(url,param,config).success(function(response){	
       		  $scope.baseServiceConfig.thirdCode = response.thirdCode;
       	  });
      }
      
      //验证三级能力编码是否存在
      $scope.checkServiceThirdCode = function(){
    	  var url = rootUrl + '/checkServiceThirdCode';
       	  var param = {"thirdCode":$scope.baseServiceConfig.thirdCode};
       	  $http.post(url,param,config).success(function(response){	
       		  //$scope.baseServiceConfig.thirdCode = response;
       		  if(response == "0"){
       			  alert("可以使用");
       		  }else{
       			alert("当前编码已经存在，不可以使用，请去数据库自定义查询");
       		  }
       	  });
      }
      
      //基础配置页面
      $scope.serviceBaseConfig = {};
      $scope.goServiceConfigPage = function(){
    	  ngDialog.open({
              template: "app/views/ngdialog-service-base-config.html",
              width: 600,
              className: 'ngdialog-theme-default',
              scope: $scope
          });
      ...
      //添加新平台页面
      $scope.serviceNewComponent = {"componentId":"","componentName":""};
      $scope.goAddServiceComponentPage = function(){
    	  ngDialog.open({
              template: "app/views/ngdialog-service-component-add.html",
              width: 600,
              className: 'ngdialog-theme-default',
              scope: $scope              
          });
      ...
      //返回新的平台编码并且验证
      $scope.findServiceComponentIdAndCheck = function(){
    	  var url = rootUrl + '/findServiceComponentIdAndCheck';
       	  var param = {"a":""};
       	  $http.post(url,param,config).success(function(response){	
       		  //$scope.baseServiceConfig.thirdCode = response;
       		  if(response.resultCode == "0"){
       			  $scope.serviceNewComponent.componentId = response.msg;
       			  alert("当前平台编码可以使用");
       		  }else{
       			alert(response.msg);
       		  }
      ...
      //添加新平台保存
      $scope.doServiceComponentAddSave = function(){
    	  var name = $scope.serviceNewComponent.componentName;
    	  ...
    	  var url = rootUrl + '/doServiceComponentAddSave';
       	  var param = $scope.serviceNewComponent;
       	  $http.post(url,param,config).success(function(response){	
       		  if(response.resultCode == "0"){
       			  alert(response.msg);
       			$scope.serviceNewComponent.componentId = response.msg;
      ...
      $scope.newService = {
    	  "serviceId":"",
    	  "serviceName":"",
    	  "serviceReq":$scope.baseServiceConfig.serviceReq,
    	  "serviceTmo":$scope.baseServiceConfig.serviceTmo,
    	  "serviceUrl":"",
    	  "serviceTwoCode":$scope.baseServiceConfig.secondCode,
    	  "servicecontractVersion":$scope.baseServiceConfig.secondCode.contractVersion,
    	  "serviceDfFromCompCode":$scope.baseServiceConfig.secondCode.dfFromCompCode,
  		  "serviceDfToCompCode":$scope.baseServiceConfig.secondCode.dfToCompCode
      };
      $scope.goServiceAddPage = function(){
    	  ngDialog.open({
              template: "app/views/ngdialog-service-add.html",
              width: 600,
              className: 'ngdialog-theme-default',
              scope: $scope              
          });
      ...
      //添加新接口保存
      $scope.doServiceAddSave = function(){
    	  if($scope.newService.serviceId == ""){
    		  alert("请填入接口编号");
    		  return ;
    	  }
    	  if($scope.newService.servicename == ""){
    		  alert("请填入接口名称");
    		  return ;
    	  }
    	  if($scope.newService.serviceUrl == ""){
    		  alert("请填入接口地址url");
    		  return ;
    	  }
    	  
    	  var url = rootUrl + '/doServiceAddSave';
       	  var param = $scope.newService;
       	  $http.post(url,param,config).success(function(response){	
       		 alert(response.msg);   		  
       	  });
      }
...
App.controller('ChartMorrisController', ['$scope', '$timeout', 'colors', function ($scope, $timeout, colors) {
  ...
  /*
	 * test data update $timeout(function(){ $scope.chartdata[0].a = 50;
	 * $scope.chartdata[0].b = 50; }, 3000);
	 */
...
App.controller('TablexEditableController', ['$scope', '$filter', '$http', 'editableOptions', 'editableThemes','$q',
  ...
  $scope.saveColumn = function(column) {
    ...
    angular.forEach($scope.users, function(user) {
      // results.push($http.post('/saveColumn', {column: column, value:
...
App.directive('classyloader', ["$timeout", "Utils", function($timeout, Utils) {
  ...
  var $scroller       = $(window),
      inViewFlagClass = 'js-is-in-view'; // a classname to detect when a
											// chart has been triggered after
											// scroll
    ...
    controller: ["$scope", "$element", function ($scope, $element) {
      ...
      $element.on('click', function () {
        ...
        function destroyPanel() {
          ...
          parent.remove();
          // remove the parent if it is a row and is empty and not a sortable
			// (portlet)
          col
            .filter(function() {
            var el = $(this);
